...
void sequential_kmeans(int num_cluster, KmeansData& data, int max_num_iter, float threshold, bool output_centroids_flag, int seed, bool verbose) {
    if (verbose) {
        std::cout << "Executing Sequential K-Means (float)..." << std::endl;
    }
    ...
    std::vector<float> old_centroids(num_cluster * dims);
    std::vector<float> new_centroids_sum(num_cluster * dims, 0.0f);
    ...
    for (int iter = 0; iter < max_num_iter; ++iter) {
        ...
        memcpy(old_centroids.data(), data.h_centroids, (size_t)num_cluster * dims * sizeof(float));
        ...
        for (int p_idx = 0; p_idx < num_points; ++p_idx) {
            ...
            for (int c_idx = 0; c_idx < num_cluster; ++c_idx) {
                ...
                for (int d_idx = 0; d_idx < dims; ++d_idx) {
                    float diff = data.h_points[p_idx * dims + d_idx] - data.h_centroids[c_idx * dims + d_idx];
        ...
        for (int c_idx = 0; c_idx < num_cluster; ++c_idx) {
            float dist_sq = 0.0f;
            for (int d_idx = 0; d_idx < dims; ++d_idx) {
                float diff = data.h_centroids[c_idx * dims + d_idx] - old_centroids[c_idx * dims + d_idx];
                dist_sq += diff * diff;
            }
...
void smem_cuda_kmeans(int num_cluster, KmeansData& data, int max_num_iter, float threshold, bool output_centroids_flag, bool verbose) {
    if (verbose) {
        std::cout << "Executing CUDA K-Means with Shared Memory (float)..." << std::endl;
    }
    ...
    size_t update_smem_size = (num_cluster * dims * sizeof(float)) + (num_cluster * sizeof(int));
...
void cuda_kmeans(int num_cluster, KmeansData& data, int max_num_iter, float threshold, bool output_centroids_flag, bool verbose) {
    if (verbose) {
        std::cout << "Executing CUDA K-Means (float)..." << std::endl;
    }
    ...
    if (params.output_centroids_flag) {
        ...
        for (int i = 0; i < data.num_centroids; ++i) {
            ...
            for (int d = 0; d < data.dims; ++d) {
                ...
                printf("%f ", data.h_centroids[i * data.dims + d]);
            }
            printf("\n");
        }
    } else {
